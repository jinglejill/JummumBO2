...
export class DiscountProgramDetailScreen extends Component
{
  ...
  constructor(props)
  {
    ...
    if(this.props.navigation.state.params.discountProgramID == 0)//new
    {
      newDiscountProgram = true;
      displayType = 0;
      if(displayType == 0)
      {
      ...
      this.state = {
        ...
        newDiscountProgram: newDiscountProgram,
        display: false,
    ...
    {
      newDiscountProgram = false;
      displayType = parseInt(this.props.navigation.state.params.displayType);
      ...
      {
        uri = this.props.navigation.state.params.dataUrl + 'JBODownloadImageGet.php?branchID='+branchID+'&imageFileName='+imageUrl+'&type=6';
        imageSource = {uri:uri};
      }
      ...
      //minimumSpend
      minimumSpend = this.props.navigation.state.params.minimumSpend;
      ...
      this.state = {
        ...
        discountProgramID:this.props.navigation.state.params.discountProgramID,
  ...
  saveDetails = () => {
    ...
    fetch(this.state.dataUrl + 'JBODiscountProgramUpdate.php',
    {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({
        ...
        discountProgramID: this.state.discountProgramID,
  ...
  amountChanged = (text) =>
  {
    ...
    this.state.discountTypeOptions.map((discountTypeOption)=>
    {
      if(discountTypeOption.value == this.state.discountType)
      {
        ...
        title = title.replace("x",this.state.minimumSpend);
        title = title.replace("y",this.state.minimumSpend*amount);
  ...
  minimumSpendChanged = (text) =>
  {
    ...
    this.setState({ minimumSpend: minimumSpend});
    this.state.discountTypeOptions.map((discountTypeOption)=>
    {
      if(discountTypeOption.value == this.state.discountType)
      {
        title = discountTypeOption.title.replace("?",this.state.amount);
        title = title.replace("x",minimumSpend);
        title = title.replace("y",this.state.amount*minimumSpend);
        this.setState({title:title});
      }
    });
  }
  ...
  {
    this.setState({showSpinner:true});
    fetch(this.state.dataUrl + 'JBODiscountProgramDelete.php',
    {
      method: 'POST',
      headers: {
                  'Accept': 'application/json',
    ...
    .then((responseData) =>{
      ...
      {
        this.setState({showSpinner:false});
        this.triggerModal("ลบส่วนลดสำเร็จ", true);
      }
    }).done();
  }
    ...
    return (
      ...
      <View style={{flex:1}}>
        ...
        <View style={{display:'flex',flexDirection:'row',alignItems:'center'}}>
          <Text style={[styles.label,{fontFamily:'Prompt-SemiBold',position:'absolute',left:null,right:10+150+20}]}></Text>
          <ButtonGroup
            onPress={(selectedIndex)=>this.displayTypeChanged(selectedIndex)}
            selectedIndex={this.state.displayType}
            buttons={segments}
            containerStyle={{height: 30,width:150,position:'absolute',right:10}}
            textStyle={{fontFamily:'Prompt-SemiBold',fontSize:14}}
            selectedButtonStyle={{backgroundColor:'#64DCC8'}}
          />
        </View>
        ...
        <Text style={styles.label}></Text>
        <View style={{display:'flex',flexDirection:'row'}}>
        <Text style={styles.label}>ชื่อส่วนลด</Text><Text style={[styles.label,{color:"#FF3C4B"}]}> *</Text>
        </View>
        <TextInput
        ...
        keyboardType="numeric"
        onChangeText={(text) => this.noOfLimitUseChanged(text)}
        value={this.state.noOfLimitUse}
        />
        ...
        placeholder=" จำนวนครั้ง"
        keyboardType="numeric"
        onChangeText={(text) => this.noOfLimitUsePerUserChanged(text)}
        value={this.state.noOfLimitUsePerUser}
        />
        ...
        placeholder=" จำนวนครั้ง"
        keyboardType="numeric"
        onChangeText={(text) => this.noOfLimitUsePerUserPerDayChanged(text)}
        value={this.state.noOfLimitUsePerUserPerDay}
        />
        <Text style={styles.label}></Text>
        ...
        <Button buttonStyle={this.state.newDiscountProgram?styles.hide:styles.button} titleStyle={{fontFamily: "Prompt-SemiBold"}} color="#000000" onPress={this.confirmDelete} title="ลบส่วนลดนี้"/>
        <Text style={styles.label}></Text>
...
export default DiscountProgramDetailScreen;
